...
BOTTOMWALL = 0
TOPWALL = 5
MAXGRIDCELLX = TOPWALL-1
MAXGRIDCELLY = RIGHTWALL-1
ACTIONSIZE = 5
MAXTIMESTEPS = 1000
IDLEPENALTY = 0
TIMESTEPPENALTY = -1
...
obs_low = np.array([0, 0, 0, 0] * AGENTS + [0,0] * (OBSTACLECOUNT), dtype=np.int32)
obs_high = np.array([MAXGRIDCELLX, MAXGRIDCELLY, MAXGRIDCELLX, MAXGRIDCELLY] * AGENTS +
                    [MAXGRIDCELLX, MAXGRIDCELLY] * OBSTACLECOUNT, dtype=np.int32)
...
class CustomEnvironment(gym.Env):
    def __init__(self, epsilon=1.0):
        ...
        #generate the random locations for the obstacles
        self.random_locations = []
        for count in range(OBSTACLECOUNT):
            obstacle_location = (random.randint(0,RIGHTWALL-1), random.randint(2,TOPWALL-1))
            while obstacle_location in self.random_locations:
                obstacle_location = (random.randint(0,RIGHTWALL-1), random.randint(2,TOPWALL-1))
            self.random_locations.append(obstacle_location)
        print('locs:', self.random_locations)

        #set obstacles
        self.obstacles = place_obstacles(self.original_drones, self.random_locations)
    ...
    def reset(self):
        ...
        goal_locations = []
        #set the first goal's x location
        goal_xSpot = 2
        #set the first drone's y location
        goal_ySpot = BOTTOMWALL
        #for each goal, set the x and y locations
        for drone in self.drones:
            drone.goalx = goal_xSpot
            drone.goaly = goal_ySpot
            #add the goal location to the goal_locations list
            goal_locations.append((goal_xSpot, goal_ySpot))
            #separate each goal x location by a unit of 2
            goal_xSpot+=2
        ...
        #add the obstacle locations to the drone_locations list 
        #set the drone's locations
        for drone in self.drones:
            drone_location = (random.randint(0,RIGHTWALL-1), random.randint(0,TOPWALL-1))
        ...
        observations = []
        for drone in self.drones:
            observations.extend([drone.x, drone.y, drone.goalx, drone.goaly])
        #add the obstacles
        obstacle_obs = []
        for obstacle in self.obstacles:
