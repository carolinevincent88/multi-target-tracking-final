...
while len(second_locations) < partial_eval:
    x1, y1 = random.randint(0, RIGHTWALL-1), random.randint(lower_obstacle_bound, TOPWALL-1)
    x2, y2 = random.randint(0, RIGHTWALL-1), random.randint(lower_obstacle_bound, TOPWALL-1)
 
    # Ensure both points are not in the same cell
    if (x1, y1) != (x2, y2):
        # Ensure neither point is taken
...
class CustomEnvironment(gym.Env):
    ...
    def reset(self, scen=0, randomReset=False):
        ...
        if self.scenario_num  >= partial_eval:
            self.scenario_num = 1
        else:
            self.scenario_num += 1
