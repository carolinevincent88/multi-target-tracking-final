...
middle = [(6,5), (7,5), (8,5), (0,5), (1,5), (3,4), (4,4), (4,5)] #8 total
obs_locations = top + middle #22 total
...
while len(second_locations) < 352: #full eval is 1406
    x1, y1 = random.randint(0, RIGHTWALL-1), random.randint(4, TOPWALL-1)
    x2, y2 = random.randint(0, RIGHTWALL-1), random.randint(4, TOPWALL-1)
...
class CustomEnvironment(gym.Env):
    ...
    def reset(self, scen=0, randomReset=False):
        ...
        for drone in self.drones:
            if randomReset:
                print("Why is this random resetting in an eval? this error is in line 209 in denseTerrainTopEvalLimited")
                drone_location = (random.randint(0,RIGHTWALL-1), random.randint(4,TOPWALL-1))
        ...
        if self.scenario_num  > 351: #full eval is 1406 so >1405
            self.scenario_num = 1
        else:
            self.scenario_num += 1
