...
if TOPWALL==7:
    ...
    open_column = 3
    lower_obstacle_bound = 1
    upper_obstacle_bound = 5
    full_eval = 306
elif TOPWALL==10:
    goal_locations = [(3, 0), (6, 0)]
    open_column = 5
    lower_obstacle_bound = 2
    upper_obstacle_bound = 8
    full_eval = 650

#set the locations for the obstacles to form a narrow passage where the top two rows and bottom two rows of the grid are open and one vertical line down the middle but everything else is an obstacle
obs_locations = []
#place obstacles everywhere except the top two rows and bottom two rows of the grid and one vertical line down the middle where x=5
for x in range(0, RIGHTWALL):
    for y in range(0, TOPWALL):
        if x != open_column and y >= lower_obstacle_bound and y < upper_obstacle_bound:
            obs_locations.append((x, y))

taken_locations = []
for goal_location in goal_locations:
    taken_locations.append(goal_location)
for obstacle in obs_locations:
    taken_locations.append(obstacle)
...
for x1, y1, x2, y2 in itertools.product(range(RIGHTWALL), range(lower_obstacle_bound, TOPWALL), repeat=2):
    # Ensure both agents are not in the same cell and not in taken locations
    if (x1, y1) != (x2, y2) and ((x1, y1) not in taken_locations) and ((x2, y2) not in taken_locations):
        first_locations.append((x1, y1))
        second_locations.append((x2, y2))

print("Number of full eval first locations:", len(first_locations))
print("Number of full eval second locations:", len(second_locations))
...
class Drone():
    ...
    #a function that checks if the drone is at the same location as other drones or obstacles 
    def check_obstacle_collisions(self, drones, obstacles):
        for drone in drones:
...
class CustomEnvironment(gym.Env):
    def __init__(self, epsilon=1.0):
        ...
        #set obstacles
        self.obstacles = place_obstacles(self.original_drones, obs_locations)
    ...
    def reset(self, scen=0, randomReset=False):
        ...
        for drone in self.drones:
            if randomReset:
                drone_location = (random.randint(0,RIGHTWALL-1), random.randint(lower_obstacle_bound,TOPWALL-1))
        ...
        if self.scenario_num  >= full_eval:
            self.scenario_num = 1
        else:
            self.scenario_num += 1
    ...
    def step(self, actions):
        ...
        if drone.check_goals(self.drones):
            ...
            for drone in self.drones:
                ...
                if drone.check_obstacle_collisions(self.drones, self.obstacles):
                    #penalize for crashing
                    drone.rewards += CRASHREWARD 
                    self.crash_count += 1
