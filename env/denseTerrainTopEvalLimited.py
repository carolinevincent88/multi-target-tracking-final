...
while len(second_locations) < 60: #full eval is 240
    x1, y1 = random.randint(0, RIGHTWALL-1), random.randint(7, TOPWALL-1)
    x2, y2 = random.randint(0, RIGHTWALL-1), random.randint(7, TOPWALL-1)
    ...
    def reset(self, scen=0, randomReset=False):
        ...
        for drone in self.drones:
            if randomReset:
                ...
                drone_location = (random.randint(0,RIGHTWALL-1), random.randint(7,TOPWALL-1))
        ...
        if self.scenario_num  > 59: #full eval is 240 so >239
            self.scenario_num = 1
        else:
            self.scenario_num += 1
